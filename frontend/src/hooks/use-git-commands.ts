...
export function useGitCommands({ addMessage, checkGitStatus }: UseGitCommandsOptions) {
  const executeGitCommand = useCallback(async (action: string, originalText: string) => {
    switch (action) {
      case 'check': {
      ...
      case 'push': {
        try {
          ...
          addMessage(`pushエラー: ${e instanceof Error ? e.message : String(e)}`, 'error');
        }
        break;
      }
      case 'pr': {
        try {
          ...
          const title = logData.log?.length > 0
            ? logData.log[0].replace(/^[a-f0-9]+\s+/, '')
            : originalText;
          const r = await fetch('/api/git/pr', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ title, body: `音声入力: ${originalText}` }),
          });
          const data = await r.json();
          if (data.success) {
          ...
          addMessage(`PR作成エラー: ${e instanceof Error ? e.message : String(e)}`, 'error');
        }
        break;
      }
      case 'branch': {
        ...
        try {
          ...
          addMessage(`ブランチ作成エラー: ${e instanceof Error ? e.message : String(e)}`, 'error');
        }
        break;
      }
      case 'log': {
        try {
          ...
          addMessage(`ログ取得エラー: ${e instanceof Error ? e.message : String(e)}`, 'error');
        }
        break;
      }
      case 'status': {
        try {
          ...
          addMessage(`ステータス取得エラー: ${e instanceof Error ? e.message : String(e)}`, 'error');
        }
        break;
      }
    }
